    ...
    <div class="navbar-header">
      <ul class="nav navbar-nav flex-row">
        <li class="nav-item mr-auto"><a class="navbar-brand" href="dashboard-analytics">
            ...
            <h2 class="brand-text mb-0">Tripser</h2>
          </a></li>
        ...
        @if(isset($menuData[1]))
          @foreach($menuData[1]->menu as $menu)
            ...
            @if(isset($menu->dropdown))
              <li
                class="@if(isset($menu->submenu)){{'dropdown'}}@endif nav-item {{ (request()->is($menu->url)) ? 'active' : '' }} {{ $custom_classes }}"
              @if(isset($menu->submenu)){{'data-menu=dropdown'}}@endif>
                <a href="{{ $menu->url }}"
                   class="@if(isset($menu->submenu)){{'dropdown-toggle'}}@endif nav-link" @if(isset($menu->submenu)){{'data-toggle=dropdown'}}@endif>
            @else
