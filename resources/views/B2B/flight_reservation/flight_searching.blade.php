...
@section('title', 'Flight Searching')
    ...
    <div class="container">
      <div class="loading">
        ...
        <div class="loading-header">
          <div class="loading-header-item" style="width: 100%">
            <i class="fa fa-map-marker"></i> <span
              class="bold-title">
              From {{$from->airport_name}} , {{$from->city_name}} ,  {{$from->country_name}}
            </span>
          </div>
          <div class="loading-header-item" style="width: 100%">
            <i class="fa fa-calendar fa-fw"></i> <span class="bold-title">
              To {{$to->airport_name}} , {{$to->city_name}} ,  {{$to->country_name}}
            </span>
          </div>

          <div class="loading-header-item" style="width: 50%">
            <i class="fa fa-map-marker"></i> <span
              class="bold-title">
              Departure: {{DateTime::createFromFormat('Y-m-d', $search_log['departure_date'])->format('M d, Y')}}
            </span>
          </div>
          <div class="loading-header-item" style="width: 50%">
            @if(isset($search_log['return_date']))
              <i class="fa fa-calendar fa-fw"></i> <span class="bold-title">
              Return: {{DateTime::createFromFormat('Y-m-d', $search_log['return_date'])->format('M d, Y')}}
            </span>
            @endif
          </div>

          <div class="loading-header-item"><i class="fa fa-user"></i><span class="bold-title"> {{$search_log['adult_count'] }} Adults</span>
          </div>
          <div class="loading-header-item"><i class="fa fa-user"></i><span class="bold-title"> {{$search_log['child_count'] }} Adults</span>
          </div>
          <div class="loading-header-item"><i class="fa fa-user"></i><span class="bold-title"> {{$search_log['infant_count'] }} Infant</span>
        ...
        <div class="modal-content">
          <div class="modal-body">
            ...
            <div class="text-center mt-3 mb-2">
              <a href="/B2B-reservation?action=flight" class="btn btn-back-to-hotel">BACK</a>
            </div>
  ...
  @section('page-script')
    <!-- Page js files -->
      ...
      <script>
        $.ajax
        ({
          url: '/B2B-flight-reservation/SearchAndAvailabilityRequest/{{$search_log->id}}',
          data: {"_token": "{{ csrf_token() }}"},
          type: 'post',
          success: function (result, textStatus, xhr) {
            if (result.result == 'OK') {
              window.location.replace("/B2B-flight-reservation/flights/{{$search_log->id}}");
            }else{
